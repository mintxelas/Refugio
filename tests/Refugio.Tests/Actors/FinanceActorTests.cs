...
public class FinanceActorTests : ActorTestBase
{
    ...
    public async Task GetAllExpenses_ReturnsAll()
    {
        await SeedAsync(db =>
        {
            db.Expenses.Add(new Expense { Description = "Food", Amount = 200m, Category = "Supplies" });
            db.Expenses.Add(new Expense { Description = "Vet", Amount = 500m, Category = "Medical" });
            return db;
        });
        var result = await _actor.Ask<List<Expense>>(new GetAllExpenses(), TimeSpan.FromSeconds(5));
        Assert.Equal(2, result.Count);
    ...
    {
        var seeded = await SeedAsync(db => { var e = new Expense { Description = "Food", Amount = 100m, Category = "Supplies" }; db.Expenses.Add(e); return e; });
    ...
    public async Task CreateExpense_CreatesAndReturns()
    {
        var result = await _actor.Ask<Expense>(
            new CreateExpense("Kennel Cleaning", 150m, "Maintenance", "monthly clean"),
            TimeSpan.FromSeconds(5));
    ...
    {
        var seeded = await SeedAsync(db => { var e = new Expense { Description = "Old", Amount = 10m, Category = "Misc" }; db.Expenses.Add(e); return e; });
        var result = await _actor.Ask<Expense?>(
            new UpdateExpense(seeded.Id, "New Desc", 999m, "Medical", null),
            TimeSpan.FromSeconds(5));
        Assert.NotNull(result);
    ...
    {
        var result = await _actor.Ask<Expense?>(
            new UpdateExpense(99999, "X", 1m, "Y", null),
            TimeSpan.FromSeconds(5));
        Assert.Null(result);
    }
    ...
    {
        var seeded = await SeedAsync(db => { var e = new Expense { Description = "Del", Amount = 1m, Category = "Misc" }; db.Expenses.Add(e); return e; });
    ...
    public async Task GetFinanceSummary_SumsDonationsAndExpenses_ForYear()
    {
        ...
        await SeedAsync(db =>
        {
            ...
            db.Expenses.Add(new Expense { Description = "Food", Amount = 200m, Category = "Supplies", Date = new DateTime(year, 3, 15) });
            return db;
        });
    ...
    public async Task GetFinanceSummary_BreaksDownByMonth()
    {
        ...
        await SeedAsync(db =>
        {
            ...
            db.Expenses.Add(new Expense { Description = "Y", Amount = 50m, Category = "Z", Date = new DateTime(year, 5, 10) });
            return db;
        });
        var result = await _actor.Ask<FinanceSummary>(new GetFinanceSummary(year), TimeSpan.FromSeconds(5));
