...
import { DONATION_CATEGORY_LABELS } from '../labels';
...
export function DonationEdit() {
  ...
  useEffect(() => {
    ...
    financeApi.getDonation(donationId)
      ...
      .catch(() => setErrors(['Donación no encontrada.']))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const errs: string[] = [];
    if (!form.donorName.trim()) errs.push('El nombre del donante es obligatorio.');
  ...
  return (
    <section className="p-margin-desktop max-w-xl mx-auto space-y-lg">
      <div className="flex items-center gap-sm">
        ...
        <h2 className="font-headline-xl text-headline-xl text-primary">{isNew ? 'Añadir donación' : 'Editar donación'}</h2>
      </div>
      <div className="bg-surface-container-lowest rounded-xl shadow-soft border border-outline-variant/30 p-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-md">
          <div><label className={LBL}>Nombre del donante *</label><input value={form.donorName} onChange={e => set('donorName', e.target.value)} className={INPUT} /></div>
          <div className="grid grid-cols-2 gap-md">
            ...
            <div><label className={LBL}>Categoría</label>
              <select value={form.category} onChange={e => set('category', e.target.value)} className={INPUT}>
                {CATEGORIES.map(c => <option key={c} value={c}>{DONATION_CATEGORY_LABELS[c]}</option>)}
              </select>
            </div>
          </div>
          <div><label className={LBL}>NIF / CIF</label><input value={form.taxId} onChange={e => set('taxId', e.target.value)} className={INPUT} /></div>
          ...
          <div className="flex gap-sm justify-end pt-sm">
            <Link to="/funds?tab=donations" className="px-md py-3 rounded-lg border border-outline-variant text-body-sm hover:bg-surface-container transition-all">Cancelar</Link>
