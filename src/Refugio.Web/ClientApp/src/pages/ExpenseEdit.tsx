...
import type { ExpensePhotoDto, ExpenseCategory } from '../types';
import { EXPENSE_CATEGORY_LABELS } from '../labels';

const CATEGORIES: ExpenseCategory[] = ['Medical', 'Food', 'Facilities', 'Supplies', 'Transport', 'Other'];
...
function validateRow(row: TaxRow): string | null {
  ...
  if (isNaN(iva) || isNaN(base) || isNaN(imp)) return 'Todos los campos son obligatorios.';
  if (base <= 0) return 'La base debe ser mayor que 0.';
  const expected = Math.round(iva / 100 * base * 100) / 100;
  if (expected !== imp) return `El importe debe ser ${iva}% × Base = ${expected.toFixed(2)}.`;
  return null;
}
...
export function ExpenseEdit() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch(() => setErrors(['Gasto no encontrado.']))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const errs: string[] = [];
    if (!form.description.trim()) errs.push('La descripción es obligatoria.');
    if (isNaN(parseFloat(form.amount)) || parseFloat(form.amount) <= 0) errs.push('El importe debe ser mayor que 0.');

    const newRowErrors = taxLines.map(validateRow);
    setRowErrors(newRowErrors);
    if (newRowErrors.some(e => e !== null)) errs.push('Corrige los errores de las líneas de IVA antes de guardar.');
  ...
  return (
    <section className="p-margin-desktop max-w-xl mx-auto space-y-lg">
      <div className="flex items-center gap-sm">
        ...
        <h2 className="font-headline-xl text-headline-xl text-primary">{isNew ? 'Añadir gasto' : 'Editar gasto'}</h2>
      </div>
      <div className="bg-surface-container-lowest rounded-xl shadow-soft border border-outline-variant/30 p-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-md">
          <div><label className={LBL}>Descripción *</label><input value={form.description} onChange={e => set('description', e.target.value)} className={INPUT} /></div>
          <div className="grid grid-cols-2 gap-md">
            <div><label className={LBL}>Importe *</label><input type="number" step="0.01" min="0" value={form.amount} onChange={e => set('amount', e.target.value)} className={INPUT} /></div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className={LBL + ' mb-0'}>Desglose IVA *</label>
              <button
                type="button"
                onClick={addRow}
            ...
            {rowErrors.map((err, i) => err && (
              <p key={i} className="text-error text-body-sm mt-1">Fila {i + 1}: {err}</p>
            ))}
          </div>

          <div className="flex gap-sm justify-end pt-sm">
            <Link to="/funds?tab=expenses" className="px-md py-3 rounded-lg border border-outline-variant text-body-sm hover:bg-surface-container transition-all">Cancelar</Link>
      ...
      {!isNew && (
        <div className="bg-surface-container-lowest rounded-xl shadow-soft border border-outline-variant/30 p-md">
          <div className="flex items-center justify-between mb-md">
            <h3 className="font-headline-md text-headline-md text-on-surface">Justificantes</h3>
            <button onClick={() => photoRef.current?.click()} className="bg-primary text-on-primary px-sm py-1.5 rounded-lg text-label-sm hover:brightness-110 flex items-center gap-xs">
              <span className="material-symbols-outlined" style={{ fontSize: 16 }}>upload</span>Subir
            </button>
