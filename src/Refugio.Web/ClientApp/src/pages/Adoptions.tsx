...
import { ADOPTION_STATUS_LABELS, ADOPTION_TYPE_LABELS } from '../labels';
...
export function Adoptions() {
  ...
  const rejectAdoption = async (id: number) => {
    if (!confirm('¿Rechazar esta adopción?')) return;
  ...
  return (
    <section className="p-margin-desktop space-y-lg">
      <div className="flex items-center justify-between">
        <h2 className="font-headline-xl text-headline-xl text-primary">Adopciones</h2>
      ...
      {showNewForm && (
        <div className="bg-surface-container-lowest rounded-xl shadow-soft border border-outline-variant/30 p-md">
          <h3 className="font-headline-md text-headline-md text-on-surface mb-md">Nueva solicitud</h3>
          <form onSubmit={createAdoption} className="grid grid-cols-2 gap-md">
            <div>
              <label className={LBL}>Perro *</label>
            ...
            <div><label className={LBL}>Nombre del solicitante *</label><input value={newForm.applicantName} onChange={e => setNewForm(f => ({ ...f, applicantName: e.target.value }))} className={INPUT} required /></div>
            ...
            <div><label className={LBL}>Teléfono</label><input value={newForm.applicantPhone} onChange={e => setNewForm(f => ({ ...f, applicantPhone: e.target.value }))} className={INPUT} /></div>
            <div><label className={LBL}>Notas</label><input value={newForm.notes} onChange={e => setNewForm(f => ({ ...f, notes: e.target.value }))} className={INPUT} /></div>
            <div className="col-span-2 flex gap-sm justify-end">
              <button type="button" onClick={() => setShowNewForm(false)} className="px-md py-2 rounded-lg border border-outline-variant text-body-sm hover:bg-surface-container transition-all">Cancelar</button>
              <button type="submit" disabled={newSaving} className="bg-primary text-on-primary px-md py-2 rounded-lg font-label-md text-label-md hover:brightness-110 disabled:opacity-60">Enviar</button>
            </div>
          </form>
        </div>
      )}
      ...
      <div className="flex gap-gutter overflow-x-auto pb-md">
          ...
          return (
            <div key={col} className="flex-shrink-0 w-64">
              <div className="flex items-center justify-between mb-sm px-1">
                <span className="font-label-md text-label-md text-on-surface">{ADOPTION_STATUS_LABELS[col]}</span>
              ...
              <div className="space-y-2">
                ...
                {cards.length > limit && (
                  <button onClick={() => showMore(col)} className="w-full py-2 text-label-sm text-primary hover:bg-primary/5 rounded-lg transition-all">
                    Ver más ({cards.length - limit} más)
                  </button>
                )}
              </div>
            </div>
        ...
        {rejected.length > 0 && (
          <div className="flex-shrink-0 w-64">
            <div className="flex items-center justify-between mb-sm px-1">
              <span className="font-label-md text-label-md text-error">{ADOPTION_STATUS_LABELS.Rejected}</span>
