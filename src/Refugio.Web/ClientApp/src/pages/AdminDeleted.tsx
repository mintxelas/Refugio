...
export function AdminDeleted() {
  ...
  const purgeDog = async (id: number) => { if (!confirm('Permanently delete?')) return; await dogsApi.purge(id); setDogs(ds => ds.filter(d => d.id !== id)); };

  const restoreMedical = async (id: number) => { await dogsApi.restoreMedicalRecord(id); setMedical(ms => ms.filter(m => m.id !== id)); };
  const purgeMedical = async (id: number) => { if (!confirm('Permanently delete?')) return; await dogsApi.purgeMedicalRecord(id); setMedical(ms => ms.filter(m => m.id !== id)); };

  const restoreMedication = async (id: number) => { await dogsApi.restoreMedication(id); setMedications(ms => ms.filter(m => m.id !== id)); };
  const purgeMedication = async (id: number) => { if (!confirm('Permanently delete?')) return; await dogsApi.purgeMedication(id); setMedications(ms => ms.filter(m => m.id !== id)); };

  const restoreAdoption = async (id: number) => { await adoptionsApi.restore(id); setAdoptions(as => as.filter(a => a.id !== id)); };
  const purgeAdoption = async (id: number) => { if (!confirm('Permanently delete?')) return; await adoptionsApi.purge(id); setAdoptions(as => as.filter(a => a.id !== id)); };

  const restoreDonation = async (id: number) => { await financeApi.restoreDonation(id); setDonations(ds => ds.filter(d => d.id !== id)); };
  const purgeDonation = async (id: number) => { if (!confirm('Permanently delete?')) return; await financeApi.purgeDonation(id); setDonations(ds => ds.filter(d => d.id !== id)); };

  const restoreExpense = async (id: number) => { await financeApi.restoreExpense(id); setExpenses(es => es.filter(e => e.id !== id)); };
  const purgeExpense = async (id: number) => { if (!confirm('Permanently delete?')) return; await financeApi.purgeExpense(id); setExpenses(es => es.filter(e => e.id !== id)); };

  const restoreGoal = async (id: number) => { await financeApi.restoreGoal(id); setGoals(gs => gs.filter(g => g.id !== id)); };
  const purgeGoal = async (id: number) => { if (!confirm('Permanently delete?')) return; await financeApi.purgeGoal(id); setGoals(gs => gs.filter(g => g.id !== id)); };

  const restoreVolunteer = async (id: number) => { await volunteersApi.restore(id); setVolunteers(vs => vs.filter(v => v.id !== id)); };
  const purgeVolunteer = async (id: number) => { if (!confirm('Permanently delete?')) return; await volunteersApi.purge(id); setVolunteers(vs => vs.filter(v => v.id !== id)); };
  ...
  return (
    <section className="p-margin-desktop space-y-lg max-w-screen-xl mx-auto">
      <div className="flex items-center gap-sm">
        ...
        <h2 className="font-headline-xl text-headline-xl text-primary">Admin — Deleted Records</h2>
      </div>
      ...
      {loading ? <LoadingSpinner /> : (
        <div className="bg-surface-container-lowest rounded-xl shadow-soft border border-outline-variant/30 overflow-hidden">
          {tab === 'dogs' && (
            <DeletedTable
              items={dogs}
              columns={['Name', 'Breed', 'Status', 'Deleted']}
          ...
          {tab === 'medical' && (
            <DeletedTable
              items={medical}
              columns={['Diagnosis', 'Vet', 'Visit Date', 'Deleted']}
          ...
          {tab === 'medications' && (
            <DeletedTable
              items={medications}
              columns={['Name', 'Dosage', 'Frequency', 'Deleted']}
          ...
          {tab === 'adoptions' && (
            <DeletedTable
              items={adoptions}
              columns={['Applicant', 'Type', 'Status', 'Deleted']}
          ...
          {tab === 'donations' && (
            <DeletedTable
              items={donations}
              columns={['Donor', 'Amount', 'Category', 'Deleted']}
          ...
          {tab === 'expenses' && (
            <DeletedTable
              items={expenses}
              columns={['Description', 'Amount', 'Category', 'Deleted']}
          ...
          {tab === 'goals' && (
            <DeletedTable
              items={goals}
              columns={['Title', 'Target', 'Current', 'Deleted']}
          ...
          {tab === 'volunteers' && (
            <DeletedTable
              items={volunteers}
              columns={['Name', 'Role', 'Email', 'Deleted']}
...
function DeletedTable<T extends { id: number }>({
  ...
  if (items.length === 0) {
    return <p className="p-md text-body-sm text-on-surface-variant text-center">No deleted records.</p>;
  }
    ...
    <table className="w-full">
      <thead className="bg-surface-container">
        <tr>
          ...
          <th className="px-md py-3 text-right text-label-md font-label-md text-on-surface-variant">Actions</th>
        </tr>
      </thead>
